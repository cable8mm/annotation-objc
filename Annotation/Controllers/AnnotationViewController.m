...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    NSData *saveTouchPointData =
        [defaults objectForKey:[NSString stringWithFormat:@"saveTouchPointData%d", self.content_id]];
    ...
    if (savedTouchPoints != nil) {
        ...
        if (self.saveTouchPoints != nil && [self.saveTouchPoints count] > 0) {
            for (int i = 0; i < [self.saveTouchPoints count] / 2; i++) {
                [self drawLine:[self.saveTouchPoints objectAtIndex:(i * 2)]
                             y:[self.saveTouchPoints objectAtIndex:(i * 2 + 1)]];
                NSLog(@"drawLine x : %@ y : %@", [self.saveTouchPoints objectAtIndex:(i * 2)],
                      [self.saveTouchPoints objectAtIndex:(i * 2 + 1)]);
            }
        }
    }
}
...
- (void)viewDidLoad
{
    ...
    UITapGestureRecognizer *drawGestureRecognition =
        [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(drawPath:)];
    drawGestureRecognition.numberOfTapsRequired = 1;
    [self.view addGestureRecognizer:drawGestureRecognition];

    //    UITapGestureRecognizer *dismissGestureRecognition = [[UITapGestureRecognizer alloc] initWithTarget:self
    //    action:@selector(showHideNavbar:)]; dismissGestureRecognition.numberOfTapsRequired = 2; [self.view.imageView
    //    addGestureRecognizer:dismissGestureRecognition];


    NSString *original_name = self.responseOsRawPicture[@"OsRawPicture"][@"original_name"];
    [self.fullsizeImage
        sd_setImageWithURL:[NSURL URLWithString:[API_SERVER_PREFIX stringByAppendingString:original_name]]
...
- (BOOL)isClosedPoint:(CGPoint)point
{
    if (self.saveTouchPoints != nil && [self.saveTouchPoints count] >= 2) {
        CGPoint initialPoint = CGPointMake([[self.saveTouchPoints objectAtIndex:0] floatValue],
                                           [[self.saveTouchPoints objectAtIndex:1] floatValue]);
        NSLog(@"Distance : %f, %F, %f, %f, %f", initialPoint.x, initialPoint.y, point.x, point.y,
              sqrt((point.x - initialPoint.x) * (point.x - initialPoint.x) +
                   (point.y = initialPoint.y) * (point.y = initialPoint.y)));

        if (sqrt((point.x - initialPoint.x) * (point.x - initialPoint.x) +
                 (point.y - initialPoint.y) * (point.y - initialPoint.y)) < 25) {
            return YES;
        }
    }
    return NO;
...
- (void)drawLine:(NSNumber *)x y:(NSNumber *)y
{
    ...
    if ([self isClosedPoint:touchPoint]) {
        touchPoint = CGPointMake([[self.saveTouchPoints objectAtIndex:0] floatValue],
                                 [[self.saveTouchPoints objectAtIndex:1] floatValue]);
    }
