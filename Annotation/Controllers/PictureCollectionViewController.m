...
@interface PictureCollectionViewController () <UINavigationControllerDelegate,
...
- (void)viewDidLoad
{
    ...
    //    NSString *url = [API_SERVER_PREFIX
    //    stringByAppendingString:@"os_raw_pictures.json?page=1&os_picture_type_id=1"];
    NSString *url =
        [NSString stringWithFormat:@"%@%@%d", API_SERVER_PREFIX,
                                   @"os_raw_pictures.json?page=1&os_picture_type_id=", self.os_picture_type_id];
    [manager GET:url
        parameters:nil
        progress:nil
...
- (IBAction)presentImagePicker:(id)sender
{
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypePhotoLibrary]) {
        ...
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"No Permission!"
                                                                       message:@"Allow Photo Library Permission."
                                                                preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *goSettingButton =
            [UIAlertAction actionWithTitle:@"Go Setting"
                                     style:UIAlertActionStyleDestructive
                                   handler:^(UIAlertAction *action) {
                                     [[UIApplication sharedApplication]
                                                   openURL:[NSURL URLWithString:@"App-Prefs:root=Privacy&path=PHOTOS"]
                                                   options:@{}
                                         completionHandler:nil];
                                   }];

        UIAlertAction *closeButton = [UIAlertAction actionWithTitle:@"Close"
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([segue.identifier isEqualToString:@"showAnnotationViewController"]) {
        ...
        AnnotationViewController *annotationViewController =
            (AnnotationViewController *)navController.topViewController;
